...
export default {
  data: new SlashCommandBuilder()
    .setName("uex")
    .setDescription("Commands for interacting with UEX.")
      ...
      subcommandGroup
        ...
        .addSubcommand((subcommand) =>
          subcommand
            .setName("items")
            .setDescription(
              "Perform UEX lookup for Star Citizen items, including ship components, weapons, and more.",
            )
            .addStringOption((option) =>
              option
                .setName("query")
                .setDescription("The item name or partial name to search for.")
                .setRequired(true),
            ),
        ),
    ),
...
async function uex_lookup_items(interaction) {
  ...
  if (replyContent.length > 2000) {
    await interaction.editReply({
      ...
      files: [
        {
          attachment: Buffer.from(itemList, "utf-8"),
          name: `uex_items_${query}.txt`,
        },
      ],
    });
    return;
