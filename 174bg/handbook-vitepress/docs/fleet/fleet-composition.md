# Fleet Composition

## Design Principles {#fleet-design-principles}

The fleet composition must adhere to the following design principles:

- **Self-Sufficiency:** We are disconnected from UEE supply lines, so we need to be able to maintain and repair our ships independently.
- **Versatility:** Our fleet must be capable of handling a wide range of missions, from combat to exploration to trade.
- **Quality over Quantity:** We will prioritize maintaining a smaller fleet of high-quality, well-maintained ships over a larger fleet of lower-quality vessels.

## Standard Issue Ships

Though we may have access to a variety of ships, the following are the standard issue ships that we will maintain in our fleet:

### Space Superiority Fighter

> High-speed dogfighter

A fast and agile fighter designed for space combat, capable of engaging enemy ships and screening for larger vessels.

- **Primary Ship:** [Scorpius](https://starcitizen.tools/Scorpius)
- **Vanduul Variant:** [Glaive (replica)](<https://starcitizen.tools/Glaive_(replica)>). For missions where we need to blend in with Vanduul forces or operate in Vanduul-controlled space.

### Anti-Capital Ship

> Armor-cracking capital ship killer

A heavily armed ship designed to take on larger capital ships, equipped with powerful weapons such as torpedoes and beam weapons able to penetrate heavy armor.

- **Primary Ship:** [Tiburon](https://starcitizen.tools/Tiburon)

### Ground Bomber

> Orbital ordnance against ground targets

A ship designed to deliver powerful ordnance against ground targets.

- **Primary Ship:** [Retaliator](https://starcitizen.tools/Retaliator)
- **Scorched-Earth Variant:** [A2 Hercules Starlifter](https://starcitizen.tools/A2_Hercules_Starlifter). For missions where we need to deny the enemy access to resources or territory, capable of delivering powerful area-of-effect weapons to destroy infrastructure and resources.

### Stealth Bomber

> Silent, precision strikes on high-value targets

A ship designed to deliver powerful ordnance against high-value targets while avoiding detection, equipped with advanced stealth technology and capable of penetrating enemy defenses to strike critical infrastructure or capital ships.

- **Primary Ship:** [Eclipse](https://starcitizen.tools/Eclipse)

### Dropship

> Armored troop transport with close air support

A ship designed to transport troops and equipment to and from the battlefield whilst providing close air support and protection, capable of deploying forces directly into combat zones without the need for an escort or support.

- **Primary Ship:** [Valkyrie](https://starcitizen.tools/Valkyrie)
- **Vehicle Variant:** [Asgard](https://starcitizen.tools/Asgard). A smaller version of the Valkyrie but with a larger cargo bay designed to transport vehicles and heavy equipment, capable of deploying forces directly into combat zones without the need for an escort or support.

### Stealth Dropship

> Silent insertion for troops and cargo

A ship designed to transport troops and equipment to and from the battlefield while avoiding detection, equipped with advanced stealth technology and capable of deploying forces directly into combat zones without the need for an escort or support.

- **Primary Ship:** [Prowler](https://starcitizen.tools/Prowler)

### Boarding Ship

> Fast-attack craft for boarding operations

A ship designed to transport boarding parties and equipment for boarding operations, able to quickly and efficiently deploy forces onto enemy vessels or structures.

- **Primary Ship:** [Legionnaire](https://starcitizen.tools/Legionnaire)

### Ship of the Line

> Heavily armed and armored fleet backbone

A heavily armed and armored ship designed to serve as the backbone of our fleet, capable of engaging enemy capital ships and providing support for smaller vessels.

- **Primary Ship:** [Perseus](https://starcitizen.tools/Perseus)

### Command Ship

> Fleet command-and-control hub

A ship designed to serve as the command center for our fleet, equipped with advanced communication and sensor systems to coordinate fleet operations and provide situational awareness as well as dedicated medical facilities to support our personnel during extended missions.

- **Primary Ship:** [Idris-K](https://starcitizen.tools/Idris-K)
- **Patrol Variant:** [Polaris](https://starcitizen.tools/Polaris). A lighter and more agile variant designed for reconnaissance and patrol missions, capable of operating independently or as part of a larger fleet.

### Carrier

> Mobile staging base for the fighter wing

A ship designed to serve as a mobile staging point for our fleet, capable of launching and recovering smaller vessels and providing support for extended missions.

- **Primary Ship:** [Kraken](https://starcitizen.tools/Kraken)

### Interdictor

> Signal-jamming battlefield disruptor

A ship designed to disrupt enemy communications and sensor systems, capable of jamming or hacking enemy vessels to gain an advantage in combat or reconnaissance.

- **Primary Ship:** [Scorpius](https://starcitizen.tools/Scorpius) Antares
- **Heavy Interdictor:** [Vanguard Sentinel](https://starcitizen.tools/Vanguard_Sentinel). A larger and more heavily armed variant designed to take more punishing and provide additional firepower to support fleet operations.

### Ambulance

> Frontline medevac and trauma care

A ship designed to provide medical support and evacuation for our personnel, equipped with advanced medical facilities and capable of transporting injured personnel to safety.

- **Primary Ship:** [Terrapin Medic](https://starcitizen.tools/Terrapin_Medic)
- **Rescue Variant:** [Cutlass Red](https://starcitizen.tools/Cutlass_Red). For search and rescue operations, capable of quickly reaching and extracting personnel from dangerous situations.
- **Field Hospital Variant:** [Apollo Medivac](https://starcitizen.tools/Apollo_Medivac). For advanced medical care during extended missions, equipped with state-of-the-art medical facilities and staffed by highly trained medical personnel.

### Minelayer

> Area-denial minefield deployment

A ship designed to deploy mines and other area-denial weapons, capable of creating defensive perimeters or disrupting enemy movements.

- **Primary Ship:** [Nautilus](https://starcitizen.tools/Nautilus)

### Snub Fighter

> Small, cheap, hit-and-run striker

A small and agile fighter designed for hit-and-run tactics, capable of quickly engaging and disengaging from enemy forces.

- **Primary Ship:** [Fury](https://starcitizen.tools/Fury)
- **Snub Bomber Variant:** [Fury MX](https://starcitizen.tools/Fury_MX). Designed to deliver overwhelming missile firepower against enemy targets, capable of quickly engaging and disengaging from combat while delivering devastating strikes.

### Anti-Swarm Ship

> Rapid-fire counter to fighter swarms

A ship designed to engage and destroy smaller aerospace threats such as enemy fighters and ordnance, equipped with multiple rapid-fire weapons and advanced targeting systems to effectively counter many targets in parallel as well as providing overwhelming suppression against ground targets.

- **Primary Ship:** [Hammerhead](https://starcitizen.tools/Hammerhead)

### Mining Vessel

> Heavy-duty asteroid resource extraction

A ship designed to extract and process resources from asteroids and other celestial bodies, equipped with advanced mining equipment and storage facilities to support sustained mining operations.

- **Primary Ship:** [Mole](https://starcitizen.tools/Mole) ([Orion](https://starcitizen.tools/Orion) once available)

### Salvage Vessel

> Wreck-stripping resource reclamation

A ship designed to extract and process resources from derelict ships and other space debris, equipped with advanced salvage equipment and storage facilities to support sustained salvage operations.

- **Primary Ship:** [Reclaimer](https://starcitizen.tools/Reclaimer)

### Mothership

> Mobile forward operating base

A large and heavily equipped ship designed to serve as a mobile base of operations for our fleet, capable of supporting extended missions and providing facilities for repair, resupply, and personnel support.

- **Primary Ship:** [Ironclad Assault](https://starcitizen.tools/Ironclad_Assault)

### Freighter

> Bulk long-haul cargo hauler

A large cargo ship designed for transporting goods and resources across the galaxy, equipped with extensive storage facilities and advanced navigation systems for long-distance travel.

- **Primary Ship:** [Hull C](https://starcitizen.tools/Hull_C) ([Hull E](https://starcitizen.tools/Hull_E) once available)

### Refueler

> In-space refuel and resupply tanker

A ship designed to provide fuel and other resources to other vessels in space, equipped with advanced refueling systems and storage facilities to support extended operations.

- **Primary Ship:** [Starfarer Gemini](https://starcitizen.tools/Starfarer_Gemini)

### Datarunner

> High-security data courier

A ship designed to transport sensitive data and information across the galaxy, equipped with advanced security systems and storage facilities to ensure the safe delivery of critical information.

- **Primary Ship:** [Herald](https://starcitizen.tools/Herald)

### Exploration Vessel

> Long-range deep-space survey ship

A ship designed for deep space exploration, equipped with advanced sensors, long-range communication systems, and living quarters to support extended missions in uncharted regions of the galaxy.

- **Primary Ship:** [Carrack](https://starcitizen.tools/Carrack)

### Repair Ship

> Mobile repair and maintenance depot

A ship designed for repairing and maintaining other vessels in space, equipped with advanced repair systems, tools, and storage facilities to support extended operations.

- **Primary Ship:** [Crucible](https://starcitizen.tools/Crucible)

### Gunship

> Maximum firepower in a minimal shipframe

A heavily armed and armored ship designed for direct combat, capable of engaging enemy forces and providing fire support for other vessels both in space and on the ground. A gunship is meant to bring as much firepower to the fight in as small of a possible shipframe.

- **Primary Ship:** [Paladin](https://starcitizen.tools/Paladin)

## Ship Loadouts

The following loadouts are the standard issue equipment for the ships of the 174th Battle Group, organized by ship type and the loadout role.

### Perseus {#ship-loadouts-perseus}

- **[Offensive Loadout:](https://www.erkul.games/loadout/CF1BphDh)** Combat-oriented loadout for high sustained DPS.

### Polaris {#ship-loadouts-polaris}

- **[Offensive Loadout:](https://www.erkul.games/loadout/tRWq4rNp)** Combat-oriented loadout for high sustained DPS.
- **[Support Loadout:](https://www.erkul.games/loadout/YISPo2JG)** Long-lasting loadout with a priority on survivability and longevity without requiring resupply.
